...
import { get_midi_file_by_hash, get_midi_from_db, load_brainz_for_midi_file, load_llm_for_midi_file, read_midi_file, save_midi_file, validationStateMidiFile } from './common';
...
export function findMidiFiles(dir: string, fileList: string[] = []): string[] {
...
export async function parseDirectory(directory: string): Promise<void> {
    ...
    for (const midiPath of midiFiles) {
        ...
        try {
            processedFiles++;
            let midifile = await parse_midi_file_path(midiPath);
            if (!midifile) {
                errorFiles++;
                console.error('Fehler beim Parsen der MIDI-Datei:', midiPath);
                console.error(`Verarbeitet: ${processedFiles}, Eingefügt: ${insertedFiles}, Fehler: ${errorFiles}, Duplikate: ${doubles}`);
                continue;
            }

            try {
                await saveMidiDocument(<IDBMidiDocument>midifile);
                insertedFiles++;
            }
            catch (err) {
                errorFiles++;
...
async function parse_midi_file_path(file_path: string): Promise<IMidiFileInformation | null> {
    if (file_path) {
        let midi_file = await read_midi_file(file_path);
        if (!midi_file) {
            return null;
        }
...
ipcMain.handle("get-midi-file-by-hash", async (_event, hash: string): Promise<IMidiFileInformation | null> => {
    return await get_midi_file_by_hash(hash);
});
...
ipcMain.handle('open-midi-file', async (): Promise<IMidiFileInformation | null> => {
    ...
    try {
        return await parse_midi_file_path(filePaths[0]);
    }
    catch (err) {
