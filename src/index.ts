...
import { initMongo, isHashInDatabase, saveMidiDocument } from './electron/mongo/mongo';
...
const logFile = path.join("log.txt");
...
async function readMidiFile(filePath: string, checkDB: boolean = true): Promise<IMidiFileInformation | null> {
    if (filePath) {
        let validationState = 'unknown';
        let midifile = MidiFile.openMidiFile(filePath);
        // first check if hash is in db
        if (midifile && midifile.hash) {
            if (await isHashInDatabase(midifile.hash)) {
                console.log('Hash already in database:', midifile.filePath);
                if (checkDB) {
        ...
        else if (!musicllm || !musicbrainz || !musicbrainz.top || musicbrainz.top.length === 0) {
            validationState = 'review';
        }
        let ret: IMidiFileInformation = {
            validationState: validationState,
            midifile: midifile ? midifile.toJSON() : null,
            musicLLM: musicllm,
            musicbrainz: musicbrainz
        }
        // generate DB entry
        // console.log('ret:', JSON.stringify(ret));
        return <IMidiFileInformation>ret; // Hier wird die Methode toJSON() aufgerufen
    }
    return null;
}
...
ipcMain.handle('open-midi-file', async (): Promise<IMidiFileInformation | null> => {
    ...
    try {
        if (filePaths[0]) {
            let ret = await readMidiFile(filePaths[0], false);
            if (ret === null) {
                return null; // hash already in db
            }
            //console.log('ret:', JSON.stringify(ret));
            if (!(ret.midifile?.hash && await isHashInDatabase(ret.midifile.hash))) {
