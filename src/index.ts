...
import { initMongo, saveMidiDocument } from './electron/mongo/mongo';
...
async function readMidiFile(filePath: string): Promise<IMidiFileInformation | null> {
    if (filePath) {
...
ipcMain.handle('open-midi-file', async (): Promise<IMidiFileInformation | null> => {
    ...
    try {
        if (filePaths[0]) {
            let ret = await readMidiFile(filePaths[0]);
